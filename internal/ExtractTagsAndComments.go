...
func ExtractTagsComments(posts []byte) (map[string]string, map[string][]string) {
	...
	for _, post := range postsMap {
		...
		for _, attachment := range attachments {
			...
			// Match Comment
			commentMatch := commentRegex.FindStringSubmatch(text)
			if len(commentMatch) >= 2 {
				comment := commentMatch[1]
				for tag, label := range labels {
					if strings.Contains(authorName, label) {
						commentsMap[tag] = append(commentsMap[tag], comment)
					}
				}
			}
		}
	...
	return extractedTags, commentsMap
}
